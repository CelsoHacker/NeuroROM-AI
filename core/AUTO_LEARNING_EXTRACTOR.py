...
class AutoLearningExtractor:
    ...
    def process_game(self, rom_path: str) -> List[Tuple[int, str]]:
        ...
        print("\n" + "=" * 60)
        print(f"🎮 PROCESSANDO: {os.path.basename(rom_path)}")
        print("=" * 60)
        ...
        if rom_hash in self.learned_games:
            ...
            pattern.game_name = os.path.basename(rom_path)
        ...
        while True:
            ...
            if choice == "1":
                ...
                if os.path.exists(rom_path):
                    ...
                    if save == 's':
                        output_file = f"{os.path.basename(rom_path)}_texts.txt"
                ...
                if self.learned_games:
                    ...
                    for game_hash, pattern in list(self.learned_games.items())[:10]:
                        print(f"   • {pattern.game_name[:30]:30} (Conf: {pattern.confidence*100:.0f}%)")
