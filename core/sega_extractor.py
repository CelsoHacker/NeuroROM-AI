...
class SegaExtractor:
    ...
    def extract_texts(self, min_length: int = 4) -> List[Dict]:
        ...
        if self.platform == 'MASTER_SYSTEM':
            print("🎮 Detectado: Sega Master System - Usando Sega Extractor")

            # 1) Tenta o extrator UNIVERSAL primeiro (CRC/tabelas + scan)
            #    (Isso evita cair no modo ponteiros quando o jogo é conhecido, como Sonic.)
            try:
                try:
                    # Quando o core é um pacote (import core.sega_extractor)
                    from .MASTER_SYSTEM_UNIVERSAL_EXTRACTOR import UniversalMasterSystemExtractor
                except Exception:
                    # Quando o arquivo está no mesmo diretório e roda “solto”
                    from MASTER_SYSTEM_UNIVERSAL_EXTRACTOR import UniversalMasterSystemExtractor

                print("🧠 Extração Universal (Master System) ativada")
                u = UniversalMasterSystemExtractor(str(self.rom_path))
                print("[DEBUG] Universal extractor loaded from:", u.__class__.__module__)

                _total = u.extract_all()
                u_results = getattr(u, "results", None) or []

                if u_results:
                    # Converte para o formato que o app já usa (offset/text/confidence/category)
                    converted = []
                    for r in u_results:
                        text = r.get("text", "")
                        ...
                        converted.append({
                            ...
                            "category": r.get("category", "Universal")
                        })

                    # Remove duplicatas exatas (mesmo texto)
                    unique = []
                    seen = set()
                    ...
                    print(f"✅ Universal: {len(unique)} textos válidos (de {_total} encontrados)\n")
                    return unique

                print("⚠️ Universal não retornou textos. Indo para ponteiros (fallback).\n")
            except Exception as e:
                print(f"❌ Erro ao usar UniversalMasterSystemExtractor: {e}")
                print("🧠 Indo para ponteiros (fallback).\n")

            # 2) Fallback: modo PRO por ponteiros
