...
try:
    ...
    print(f"[ERROR]Erro ao importar gemini_translator: {e}")
...
try:
    ...
    print(f"[ERROR]Erro ao importar EngineDetector: {e}")
...
try:
    ...
    print(f"[ERROR]Erro ao importar RTCE: {e}")
    RTCEEngine = None
...
try:
    ...
    print(f"[ERROR]Erro ao importar gemini_api: {e}")
    gemini_api = None
...
try:
    ...
    print(f"[ERROR]Erro ao importar SecurityManager: {e}")
    SecurityManager = None

# Import Universal Master System Extractor (para jogos Master System)
try:
    # O arquivo MASTER_SYSTEM_UNIVERSAL_EXTRACTOR_FIXED.py deve estar dentro da pasta core
    from core.MASTER_SYSTEM_UNIVERSAL_EXTRACTOR_FIXED import UniversalMasterSystemExtractor
    print("[OK]UniversalMasterSystemExtractor importado com sucesso")
except ImportError as e:
    print(f"[ERROR]Erro ao importar UniversalMasterSystemExtractor: {e}")
...
try:
    ...
    print(f"[ERROR]Erro ao importar GraphicLabTab: {e}")
    GraphicLabTab = None
...
try:
    ...
    try:
        ...
        print(f"[ERROR]Erro ao importar Forensic Engine Tier 1: {e1} | {e2}")
...
class RTCEWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(f"[ERROR] Erro no Motor: {e}")
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(f"[ERROR] Erro crítico no RTCE: {str(e)}")
            ...
            for i, line in enumerate(lines):
                ...
                if len(current_batch) >= batch_size or i == total_lines - 1:
                    ...
                    try:
                        ...
                        self.log_signal.emit(f"[ERROR] Erro no processamento do lote: {str(e)}")
                        for orig in batch_original_lines:
                            translated_lines.append(orig + "\n")
                        current_batch = []
                        batch_original_lines = []
            ...
            for i, line in enumerate(lines):
                ...
                if len(current_batch) >= batch_size or i == total_lines - 1:
                    ...
                    try:
                        ...
                        self.log_signal.emit(f"[ERROR] Erro no lote: {str(e)}")
                        translated_lines.extend([l + "\n" for l in current_batch])
                        current_batch = []
        ...
        try:
            ...
            if not ollama_running:
                ...
                try:
                    ...
                    self.error_signal.emit(f"[ERROR] Erro ao iniciar: {str(e)}\n\nAbra CMD e execute: ollama serve")
                    return
            ...
            try:
                ...
                self.log_signal.emit(f"[WARN] Não foi possível verificar modelos instalados: {str(e)}")
...
{texts_to_translate}"""
                ...
                try:
                    ...
                    self.log_signal.emit(f"[WARN] Erro: {str(e)}")
                    errors += 1
                    translated_lines.extend(batch)
...
class ReinsertionWorker(QThread):
                ...
                for i, line in enumerate(lines):
                    ...
                    if line.startswith('['):
                        try:
                            ...
                            self.log_signal.emit(
                                f"[WARN] Erro de offset hexadecimal na linha {i+1}: {e}. Pulando."
                            )
                            continue
                        except Exception as e:
                            self.log_signal.emit(
    ...
    def _fix_snes_checksum(self, rom_path: str):
        ...
        try:
            ...
            self.log_signal.emit(f"[WARN] Erro ao corrigir checksum: {e}")
    ...
    def _reinsert_sega_rom(self):
        ...
        try:
            ...
            self.log_signal.emit(f"[WARN] Erro de importação: {e}")
    ...
    def load_translations(cls, lang_code: str) -> Dict:
        ...
        if json_file.exists():
            try:
                ...
                print(f"[WARN] Failed to load {lang_code}.json: {e}")
...
class RTCEWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(f"[ERROR] Erro no Motor RTCE: {str(e)}")
            self.error_signal.emit(str(e))
...
class MainWindow(QMainWindow):
    ...
    def clear_translation_cache(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.log(f"[ERROR] Erro ao limpar cache: {e}")
    ...
    def init_ui(self):
        ...
        self.log("[START] Sistema v6.0 [RUNTIME INTELLIGENT] Iniciado - Modo Especialista")
    ...
    def select_rom(self):
        ...
        if file_path:
            ...
            self.log(f"[OK] ROM selecionada: {os.path.basename(file_path)}")
    ...
    def detect_and_display_engine_async(self, file_path):
        ...
        try:
            ...
            self.log(f"[WARN] Erro na detecção: {e}")
    ...
    def start_engine_detection_async(self, file_path):
        ...
        try:
            ...
            self.log(f"[WARN] Erro ao iniciar detecção: {e}")
            self.engine_detection_scroll.setVisible(False)
    ...
    def on_engine_detection_complete(self, detection_result):
        ...
        try:
            ...
            error_msg = f"[WARN] Erro ao processar detecção: {e}"
            self.log(error_msg)
    ...
    def sync_platform_combobox(self, platform_code):
        ...
        try:
            ...
            self.log(f"[WARN] Erro ao sincronizar ComboBox: {e}")
    ...
    def preencher_lista_laboratorio(self, lista_caminhos):
        ...
        try:
            ...
            self.log(f"[ERROR] Erro ao processar gráficos: {e}")
    ...
    def load_extracted_txt_directly(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Extracted Text Files (*.txt *_clean_blocks.txt *_extracted.txt *_CLEAN_EXTRACTED.txt *_V98_FORENSIC.txt *_V9_EXTRACTED.txt *_extracted_texts.txt);;All Files (*.*)"
        )
        ...
        self.trans_file_label.setText(os.path.basename(file_path))
        self.trans_file_label.setStyleSheet("color: #4CAF50; font-weight: bold;")
        self.log(f"[OK] Arquivo TXT carregado: {os.path.basename(file_path)}")
        ...
        if not rom_found:
            self.log(f"[WARN] ROM original não encontrada para: {rom_name}")
    ...
    def extract_texts(self):
        ...
        self.log(f"[START] Iniciando extração: {os.path.basename(self.original_rom_path)}")
        ...
        try:
            ...
            self.log(f"[ERROR] Erro na extração: {e}")
    def start_extraction_process(self):
        """Inicia o extrator apropriado baseado na plataforma selecionada."""
        self.log(f"[START] Preparando extração: {os.path.basename(self.original_rom_path)}")
        ...
        try:
            ...
            self.log(f"[ERROR] Erro ao lançar: {e}")
        ...
        if hasattr(self, 'current_process') and self.current_process.poll() is not None:
            ...
            if os.path.exists(report_file):
                ...
                try:
                    ...
                    self.log(f"[WARN] Erro ao ler relatório: {e}")
                self.log("="*40)
    ...
    def run_batch_test(self):
        ...
        try:
            ...
            self.log(f"[ERROR] Erro ao iniciar teste em lote: {e}")
    ...
    def on_optimization_finished(self, output_file: str):
        ...
        try:
            ...
            self.log(f"[WARN] Erro ao contar linhas: {str(e)}")
    ...
    def on_fast_extract_finished(self, results: dict):
        ...
        try:
            ...
            if output_file and os.path.exists(output_file):
                ...
                self.log(f"[OK] NEUROROM AI V 6.0 PRO SUITE: Extração concluída!")
            ...
            self.log(f"[ERROR] Erro ao processar resultado: {e}")
            self.optimize_btn.setEnabled(False)
