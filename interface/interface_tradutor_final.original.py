...
from core.MASTER_SYSTEM_COMPLETE_DATABASE import CompleteMasterSystemExtractor
...
class GeminiWorker(QThread):
    ...
    def restore_tags(self, text: str) -> str:
        ...
        # Remove placeholders que o modelo possa ter inventado (se sobrar algum após restore)
        restored = re.sub(r"__PROTECTED_\\d+__", "", restored)
...
class MainWindow(QMainWindow):
    ...
    def start_extraction_process(self):
        ...
        if ext == ".sms":
            self.log("🧠 Extração PRO baseada em ponteiros ativada")

            from core.sega_extractor import SegaExtractor

            extractor = SegaExtractor(self.original_rom_path)
            texts = extractor.extract_texts(min_length=4)

            if not texts:
                self.log("⚠️ SMS PRO: Nenhum texto confiável encontrado.")
                return

            self.log(f"✅ SUCESSO: {len(texts)} strings reais extraídas.")
        ...
        try:
            self.log(f"🚀 Rodando extrator: {os.path.basename(script_path)} ... (Aguarde)")
