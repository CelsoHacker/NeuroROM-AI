...
# Configure o caminho do Tesseract para Windows CORRETAMENTE
    ...
    def run(self):
        try:
            ...
            for i, line in enumerate(lines):
                ...
                # 6.5: Caracteres especiais excessivos (>25% mais rigoroso)
                if not is_garbage:
                    special_chars = set('!@#$%^&*()_+-=[]{}|;:,.<>?/~`\\')
                    special_count = sum(1 for char in clean_text if char in special_chars)
                    if len(clean_text) > 0 and special_count / len(clean_text) > 0.25:
                        stats['binary_garbage'] += 1
                        is_garbage = True
                ...
                if not is_garbage and len(clean_text) >= 4:
                    ...
                    # Se tiver pelo menos 1 vogal mas ratio < 0.15 (ex: "iHJJJJ" = 1/5 = 0.2)
                ...
                if not is_garbage and 4 <= len(clean_text) <= 6:
                    # Se tiver caracteres especiais em string curta, provável lixo
                ...
                if is_garbage and any(word in clean_text.lower() for word in game_words):
                    is_garbage = False  # EXCEÇÃO: É palavra de jogo, aceitar!
                    stats['binary_garbage'] -= 1  # Corrigir contador

                # EXCEÇÃO 2: Textos longos com espaços (frases) são geralmente válidos
                if is_garbage and ' ' in clean_text and len(clean_text.split()) >= 2:
                    # Se tiver pelo menos 2 palavras separadas por espaço, provavelmente é válido
                    is_garbage = False
                    stats['binary_garbage'] -= 1
            ...
            def translate_single(index, text):
                ...
                for attempt in range(MAX_RETRIES):
                    try:
                        ...
                        # Prompt SIMPLES E DIRETO para todos os modelos
                        # Evita respostas truncadas e melhora qualidade
                        prompt = (
                            f"Translate to {self.target_language}: {original_text}\n\n"
                        ...
                        payload = {
                            ...
                            "stream": False,
                            "options": {
                                "temperature": 0.3,      # Criatividade moderada (evita repetir original)
                                "num_predict": 50,       # Curto = suficiente para 1-3 palavras
                                "top_p": 0.9,            # Mais opções válidas
                                "repeat_penalty": 1.1,   # Leve penalidade
                                "num_ctx": 256           # Contexto mínimo = mais rápido
                            }
                        }

                        # Timeout dinâmico baseado no modelo
                        timeout_value = 120 if "mistral" in self.model.lower() else 60
...
class ReinsertionWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # ================================================================
            # DETECÇÃO AUTOMÁTICA: PC Game (.exe) vs ROM de Console
            # ================================================================
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Translation language settings (for multi-language support)
        self.source_language_code = "auto"  # Default: auto-detect
        self.source_language_name = "AUTO-DETECT"
        self.target_language_code = "pt"  # Default: Portuguese
        self.target_language_name = "Portuguese (Brazil)"
    ...
    def init_ui(self):
        ...
        if GraphicLabTab:
            ...
            placeholder_layout.addWidget(QLabel("Graphics Lab module not available"))
            self.tabs.addTab(placeholder_tab, self.tr("tab5"))
        ...
        restart_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        exit_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
    ...
    def create_extraction_tab(self) -> QWidget:
        ...
        self.platform_combo.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        # 1.5. AVISO "EM FASE DE TESTES" (aparece para consoles)
        ...
        self.console_warning_widget.setVisible(False)  # Oculto por padrão
        ...
        self.select_rom_btn.setCursor(Qt.CursorShape.PointingHandCursor) # MÃOZINHA ATIVA
        ...
        # --- PAINEL DE ANÁLISE FORENSE (RAIO-X) ---
        self.forensic_analysis_btn = QPushButton("🔍 Análise Forense (Raio-X)")
        self.forensic_analysis_btn.setMinimumHeight(35)
        self.forensic_analysis_btn.setCursor(Qt.CursorShape.PointingHandCursor) # MÃOZINHA ATIVA
        ...
        self.forensic_progress.setRange(0, 0)
        rom_layout.addWidget(self.forensic_progress)

        # O VISOR COM BORDA LARANJA
        self.engine_detection_scroll = QScrollArea()
        self.engine_detection_scroll.setWidgetResizable(True)
        self.engine_detection_scroll.setMinimumHeight(250)
        self.engine_detection_scroll.setStyleSheet("""
            QScrollArea {
                border: 2px solid #333; /* Borda Cinza Escura, mais profissional que a laranja */
                border-radius: 8px;
                background-color: #121212;
            }
            QScrollBar:vertical {
                background: #000000; /* Fundo da barra PRETO */
                width: 12px;
            }
            QScrollBar::handle:vertical {
                background: #2a2a2a; /* Botão da barra CINZA ESCURO */
                border-radius: 6px;
            }
        """)
        ...
        self.engine_detection_label.setStyleSheet("color: #FF9800; padding: 10px; font-family: 'Consolas';")
        ...
        # 4. BOTÕES DE AÇÃO (LINDOS E EM PORTUGUÊS)
        self.extract_btn = QPushButton("📄 Extrair Textos")
        self.extract_btn.setMinimumHeight(55)
        self.extract_btn.setStyleSheet("background-color: #4CAF50; color: white; font-size: 14pt; font-weight: bold; border-radius: 8px;")
        self.extract_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        self.extract_btn.clicked.connect(self.extract_texts)
        layout.addWidget(self.extract_btn)

        self.optimize_btn = QPushButton("🧹 Otimizar Dados")
        self.optimize_btn.setMinimumHeight(55)
        self.optimize_btn.setStyleSheet("background-color: #FF9800; color: white; font-size: 14pt; font-weight: bold; border-radius: 8px;")
        self.optimize_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        self.optimize_btn.clicked.connect(self.optimize_data)
        layout.addWidget(self.optimize_btn)

        # 5. BARRAS DE PROGRESSO COM %
        self.extract_progress_bar = QProgressBar()
        self.extract_progress_bar.setFormat("Progresso de Extração: %p%")
        layout.addWidget(self.extract_progress_bar)

        self.optimize_progress_bar = QProgressBar()
        self.optimize_progress_bar.setFormat("Progresso de Otimização: %p%")
        layout.addWidget(self.optimize_progress_bar)
        # --- ADICIONE ISSO PARA O PROGRAMA PARAR DE FECHAR ---
    ...
    def create_translation_tab(self) -> QWidget:
        ...
        self.sel_file_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        self.source_lang_combo.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        self.target_lang_combo.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        # Connect language combo boxes to update internal state
        ...
        self.mode_combo.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        self.mode_combo.addItems([
            "🤖 Auto (Gemini → Ollama)",
            "⚡ Online Gemini (Google API)",
            "🚀 Offline Rápido (Llama 3.2)",
            "🔥 Offline Alta Qualidade (Mistral 7B) [RECOMENDADO]",
            "🌐 Online DeepL (API)"
        ])
        self.mode_combo.setCurrentText("🔥 Offline Alta Qualidade (Mistral 7B) [RECOMENDADO]")
        ...
        self.workers_spin.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        self.timeout_spin.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        self.translate_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        self.stop_translation_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
    ...
    def create_reinsertion_tab(self) -> QWidget:
        ...
        select_reinsert_rom_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        select_translated_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
        ...
        self.reinsert_btn.setCursor(Qt.CursorShape.PointingHandCursor)  # Cursor de mãozinha
    ...
    def select_rom(self):
        # Destravar combobox antes de selecionar novo arquivo
        self.platform_combo.setEnabled(True)

        # ESTE RECUO ABAIXO É OBRIGATÓRIO (Pressione TAB aqui)
        filtros = (
            "PC Games & PS1 (*.exe *.wad *.dat *.mtf *.box *.spt *.img *.bin *.iso *.cue);;"
        ...
        if file_path:
            # CORREÇÃO: Usar ESTRITAMENTE o arquivo selecionado pelo usuário
            # DESATIVADO: Auto-mining que mudava o arquivo sem permissão
            self.original_rom_path = file_path

            # Log de confirmação do arquivo selecionado
            self.log(f"[SISTEMA] Arquivo confirmado: {Path(file_path).name}")

            # Atualização da Interface
            nome_exibicao = Path(self.original_rom_path).name
            self.rom_path_label.setText(nome_exibicao)
            self.rom_path_label.setStyleSheet("color:#4CAF50;font-weight:bold;")
            self.reinsert_rom_label.setText(nome_exibicao)
            self.reinsert_rom_label.setStyleSheet("color:#4CAF50;font-weight:bold;")
            self.log(f"ROM/Jogo selecionado: {nome_exibicao}")

            # ================================================================
            # AUTO-SINCRONIZAÇÃO DO COMBOBOX (TRAVA PLATAFORMA CORRETA)
            # ================================================================
            file_ext = os.path.splitext(file_path)[1].lower()

            # Detecta plataforma automaticamente pela extensão
            if file_ext in ['.exe', '.dll']:
                # PC Game - forçar sincronização para PC
                self.sync_platform_combobox('PC')
                self.platform_combo.setEnabled(False)  # TRAVA combobox
                self.log("🔒 Plataforma travada: PC Games (Windows) - arquivo .exe detectado")
            elif file_ext in ['.smc', '.sfc']:
                # SNES - forçar sincronização para SNES
                self.sync_platform_combobox('SNES')
                self.platform_combo.setEnabled(False)  # TRAVA combobox
                self.log("🔒 Plataforma travada: Super Nintendo (SNES) - arquivo .smc/.sfc detectado")
            else:
                # Extensão desconhecida - deixar livre (mas avisar)
                self.platform_combo.setEnabled(True)
                self.log(f"⚠️ Extensão desconhecida ({file_ext}) - selecione a plataforma manualmente")

            # Habilitar botão de Análise Forense (Raio-X)
            self.forensic_analysis_btn.setEnabled(True)
            self.log("🔬 Análise Forense (Raio-X) disponível - clique para analisar")

            # DESABILITADO: Detecção automática (agora apenas manual via botão)
            # self.start_engine_detection_async(self.original_rom_path)
            ...
            # Validação de compatibilidade plataforma/arquivo
            if hasattr(self, 'detected_engine') and self.detected_engine:
                selected_platform = self.platform_combo.currentText()
                is_valid, error_message = self.validate_file_platform_match(
                    self.detected_engine,
                    selected_platform
                )

                if not is_valid:
                    # AVISO APENAS (não bloqueia - usuário tem palavra final)
                    ...
                    # NÃO bloqueia - usuário pode clicar em "Extrair" normalmente
                else:
                    # Arquivo compatível!
                    self.log(f"✅ Arquivo compatível com {selected_platform}")
    ...
    def on_engine_detection_complete(self, detection_result):
        ...
        try:
            ...
            # ================================================================
            # FINALIZAÇÃO DA ANÁLISE FORENSE
            # ================================================================
            self.forensic_progress.setVisible(False)
            self.forensic_analysis_btn.setEnabled(True)
    ...
    def reinsert(self):
        ...
        # SUPORTE EXPANDIDO: PC Games + ROMs de Console
        valid_extensions = ('.smc', '.sfc', '.bin', '.nes', '.z64', '.n64', '.gba', '.gb', '.gbc', '.nds', '.iso', '.exe', '.dll', '.dat')
    ...
    def extract_texts(self):
        ...
        # ================================================================
        # SELEÇÃO AUTOMÁTICA DO EXTRATOR BASEADO NA PLATAFORMA
        # ================================================================
        # Detecta tipo de arquivo pela extensão
        file_ext = os.path.splitext(self.original_rom_path)[1].lower()

        # PC Games (.exe, .dll) → generic_pc_extractor.py
        if file_ext in ['.exe', '.dll']:
            script_extractor = os.path.join(base_path, "generic_pc_extractor.py")
            self.log("🖥️ Detectado: PC Game (Windows) - usando extrator PC")

        # Consoles SNES (.smc, .sfc) → generic_snes_extractor.py
        elif file_ext in ['.smc', '.sfc']:
            script_extractor = os.path.join(base_path, "generic_snes_extractor.py")
            self.log("🎮 Detectado: SNES ROM - usando extrator SNES")

        # Fallback: usa SNES (padrão antigo)
        else:
            script_extractor = os.path.join(base_path, "generic_snes_extractor.py")
            self.log(f"⚠️ Extensão desconhecida ({file_ext}) - usando extrator SNES (fallback)")
    ...
    def translate_texts(self):
        ...
        if mode_index == 0:  # Auto (Gemini → Ollama)
            ...
            self.translate_thread = OllamaWorker(input_file, target_lang_name, model="llama3.2:1b")
    ...
    def show_graphics_guide(self):
        ...
        dialog.setWindowTitle(self.tr("manual_gfx_title"))
        dialog.setMinimumSize(700, 600)
    ...
    def run_forensic_analysis(self):
        """Dispara a análise profunda de DNA do arquivo (Raio-X)."""
        ...
        self.log("🔬 Iniciando Análise Forense TIER 1...")
        ...
        try:
            ...
            self.log(f"❌ Erro ao iniciar análise: {str(e)}")
            self.forensic_progress.setVisible(False)
            self.forensic_analysis_btn.setEnabled(True)


    # --- CÓDIGO CORRIGIDO (SUBSTITUA O FINAL DO ARQUIVO POR ISSO) ---

       # --- COLE ESTE BLOCO NO FINAL DO ARQUIVO (NA MARGEM ESQUERDA) ---
