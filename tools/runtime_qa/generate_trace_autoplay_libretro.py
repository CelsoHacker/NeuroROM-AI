...
# -*- coding: utf-8 -*-
"""
Gera script Python de Runtime Trace + Autoplay para runtime libretro.
"""
...
from tools.runtime_qa.libretro_runtime_runner import run_trace

result = run_trace(Path(r"{cfg}"))
print(json.dumps(result, ensure_ascii=False, indent=2))
"""


def _build_buffer_candidates(hook_profile: Dict[str, Any], limit: int = 64) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for row in hook_profile.get("buffer_candidates", []) or []:
        if not isinstance(row, dict):
            continue
        ptr = str(row.get("ptr_or_buf", "") or "").strip()
        if not ptr:
            continue
        out.append(
            {
...
def build_trace_payload(
    ...
    if hook_profile.get("rom_size"):
        try:
            rom_size = int(hook_profile.get("rom_size"))
        except Exception:
            pass
    ...
    return {
        "schema": "runtime_trace_config.libretro.v1",
        ...
        "max_bytes_per_capture": int(profile.get("max_bytes_per_capture", 224)),
        "default_terminators": profile.get("default_terminators", [0]),
        "autoplay_sequence": profile.get("autoplay_sequence", []),
        "seed_fallback": extract_seed_items(rows, limit=seeds_fallback, only_safe=True),
        "buffer_candidates": _build_buffer_candidates(hook_profile, limit=64),
        "runtime_trace_path": str(out_dir / f"{crc}_runtime_trace.jsonl"),
    }


def write_trace_artifacts(payload: Dict[str, Any], project_root: Path) -> Dict[str, str]:
    out_dir = Path(payload["out_dir"])
    config_path = out_dir / f"{payload['rom_crc32']}_runtime_trace_config.libretro.json"
    script_path = out_dir / f"{payload['rom_crc32']}_runtime_trace_autoplay.py"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Gera script Runtime Trace + Autoplay para runtime libretro.")
    ...
    ap.add_argument("--max-frames", type=int, default=24000, help="Frames maximos de trace")
    ap.add_argument("--sample-every-frames", type=int, default=8, help="Periodicidade de amostra")
    ap.add_argument("--seeds-fallback", type=int, default=96, help="Seeds fallback por trace")
    ...
    artifacts = write_trace_artifacts(payload, project_root=Path(__file__).resolve().parents[2])
